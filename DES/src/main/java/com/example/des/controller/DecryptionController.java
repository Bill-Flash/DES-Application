    ...
    private void addMediaListener(){
        ...
        media.getSelectionModel().selectedIndexProperty().addListener((ov, value, new_value) -> {
            if (new_value.intValue() == 1) {
                ...
                inputLabel.setText("Enter your cipher text:");
    ...
    public void decryption() {
        ...
        if (method.equals(EnumMedia.FILE)) {
            if (file[0] != null && key.getText() != null) {
                ...
                Des.DesFile(file[0].getAbsolutePath(), EnumMode.DECRYPTION);
                // tell user where it is generated
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setTitle("Successfully Decryption.");
                alert.setHeaderText(null);
                alert.setContentText("Please check the 'Decryption' fold!");
                alert.showAndWait();
            ...
            if (input != null && key.getText() != null) {
                ...
                desUtil.setKey(key.getText());
                // get the result
                result = Des.DesText(input, EnumMode.DECRYPTION);
                if (result.equals("false")){
                    Alert alert = new Alert(Alert.AlertType.WARNING);
                    alert.setTitle("Warning Dialog");
                    alert.setHeaderText(null);
                    alert.setContentText("Incorrect key or input!");
                    alert.showAndWait();
                }
